...
| Fans | confirmed base K2 schedule | No exact tool stated | Clean weekly with printer powered off; do not invent cover removal | Fan starts normally without rubbing/noise and airflow path is unobstructed | Blade/cable damage, rubbing, fan error, inaccessible debris, or disassembly not covered by source | [P027] |
| Air filter | confirmed base K2 schedule | Exact filter part number/tool not stated | Replace at the documented interval only after opening the exact official part/procedure | Filter is correctly seated; fan path is unobstructed; no warning/noise after restart | Part identity, orientation, access, or fastener procedure is unconfirmed | [P003] [P027] |
| Wiper dome/tape | confirmed base K2 schedule | Exact replacement part/tool not stated | Inspect dome daily and tape weekly; remove only safely accessible debris | Wiper is present, seated, and unobstructed | Wiper is torn/loose, replacement identity is unknown, or access requires unsupported disassembly | [P027] |
...
Creality publishes no explicit supersession statement in the cited pages. Record actual usage and source revision. Do not average the intervals or silently choose the longest. If the newer page cannot be confirmed for the physical model, use the exact model manual for routine scheduling and ask Creality support to resolve the conflict before applying a different lubricant or cadence.
...
- Do not substitute grease, oil, solvent, adhesive, or cleaner by similarity. The cited sources provide no complete chemical-compatibility list.
...
- The cited sources provide no torque for any reassembled fastener. If removal requires a torque-controlled fastener and the procedure provides no torque, stop.
