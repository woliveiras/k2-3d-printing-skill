# Reference index

Load only the domain required by the task.

| Need | Read first | Then load |
|---|---|---|
| Current claim or conflict | [Sources](sources.md) | [Evidence and authority](evidence-and-authority.md) |
| Physical printer identity or limits | [Printer identity](printer-identity.md) | Relevant official source entry |
| Hot, moving, electrical, emissions, domestic, drying, solvent, or unattended risk | [Safety](safety.md) | Exact material SDS/manual |
| STL, STEP, 3MF, G-code, scale, mesh, fit, or artifact mutation | [Model inspection](model-inspection.md) | [Design for FDM](design-for-fdm.md) |
| Filament choice and hardware/CFS/nozzle compatibility | [Materials index](materials/INDEX.md) | One family file and [matrix](materials/compatibility-matrix.md) |
| Orientation, supports, parameters, calibration, Preview, or print defect | [Slicing index](slicing/INDEX.md) | One task file |
| Exact Creality Print control, screen, mode, or version | [Creality Print index](creality-print/INDEX.md) | One version-scoped screen file |
| Preventive work, hardware symptom, or repair | [Maintenance index](maintenance/INDEX.md) | One subsystem file and [trees](maintenance/troubleshooting-trees.md) |

Use source IDs such as `S001` from `sources.md`; do not copy the same source fact into several files. Keep recommendations marked as official limit, manufacturer range, starting point, empirical adjustment, or unvalidated.
